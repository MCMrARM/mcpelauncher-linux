            ...
            if (e.code >= ABS_X && e.code < ABS_X + 16) {
                ...
                if (entry.stick != -1 && GameControllerManager::sGamePadManager != nullptr) {
                    ...
                    if (entry.stickY)
                        ...
                        val.x = value;
                    GameControllerManager::sGamePadManager->feedStick(index, entry.stick, (GameControllerStickState) 3,
                                                                      val.x, -val.y);
                }
                ...
                if (entry.button != -1 && GameControllerManager::sGamePadManager != nullptr) {
                    auto btnState = value >= 0.9f ? GameControllerButtonState::PRESSED :
                                    GameControllerButtonState::RELEASED;
                    GameControllerManager::sGamePadManager->feedButton(index, entry.button, btnState, true);
                }
            }
...
void LinuxGamepadManager::Device::onButton(MappingInfo::Entry const& mapping, bool pressed) {
    ...
    if (mapping.button != -1) {
        auto btnState = pressed ? GameControllerButtonState::PRESSED : GameControllerButtonState::RELEASED;
        GameControllerManager::sGamePadManager->feedButton(index, mapping.button, btnState, true);
    }
    if (mapping.stick != -1) {
        if (mapping.stickY)
            sticks[mapping.stick].y = mapping.stickVal;
        else
            sticks[mapping.stick].x = mapping.stickVal;
    }
